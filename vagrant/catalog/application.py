...
def editFoodGroup(foodgroup_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editFoodGroup.html',
                               foodgroup_id=foodgroup_id, item=foodgroup)
...
def deleteFoodGroup(foodgroup_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteFoodGroup.html',
                               item=foodgroup, login=login, itemid=itemid)
...
def showFoodItem(foodgroup_id):
    foodgroup = session.query(FoodGroup).filter_by(id=foodgroup_id).one()
    fooditems = session.query(FoodItem).filter_by(foodgroup_id=foodgroup_id)\
        .all()
    creator = getUserInfo(foodgroup.user_id)
    if 'username' not in login_session or \
            creator.id != login_session['user_id']:
        return render_template('publicfoodItem.html',
                               fooditems=fooditems,
                               foodgroup=foodgroup,
                               creator=creator)
    else:
        return render_template('foodItem.html', fooditems=fooditems,
                               foodgroup=foodgroup, creator=creator)

# Search for a food item in usda database


@app.route('/foodgroup/<int:foodgroup_id>/items/search/',
           methods=['GET', 'POST'])
def searchFoodItem(foodgroup_id):
    if request.method == 'POST':
        item = request.form['foodName']
        url = 'https://api.nal.usda.gov/ndb/search/?format=json&q='\
              + item + '&max=1&offset=0&api_key=' + usdakey
        response = urllib2.urlopen(url)
        data = json.load(response)
        ndbno = data['list']['item'][0]['ndbno']
        newurl = 'https://api.nal.usda.gov/ndb/reports/?ndbno=' + ndbno + \
                 '&type=b&format=json&api_key=' + usdakey
        newResponse = urllib2.urlopen(newurl)
        newData = json.load(newResponse)
        return render_template('response.html',
                               response=newData, foodgroup_id=foodgroup_id)
    else:
        return 'get request'


# Add searched food item
@app.route('/foodgroup/<int:foodgroup_id>/items/response/', methods=['\
    GET', 'POST'])
...
# Edit a food item
@app.route('/foodgroup/<int:foodgroup_id>/items/<int:fooditem_id>/edit',
           methods=['GET', 'POST'])
def editFoodItem(foodgroup_id, fooditem_id):
    ...
    if login_session['user_id'] != foodgroup.user_id:
        return "<script>function myFunction() {alert('You are \
         not authorized to edit menu items to this foodgroup. \
          Please create your own foodgroup in order to edit items.');}\
    ...
    if request.method == 'POST':
        ...
        return render_template('editFoodItem.html',
                               foodgroup_id=foodgroup_id,
                               fooditem_id=fooditem_id, item=editedItem)


# Delete a food item
@app.route('/foodgroup/<int:foodgroup_id>/item/<int:fooditem_id>/delete',
           methods=['GET', 'POST'])
def deleteFoodItem(foodgroup_id, fooditem_id):
    ...
    if login_session['user_id'] != foodgroup.user_id:
        return "<script>function myFunction() {alert('You are not \
         authorized to delete menu items to this foodgroup. \
         Please create your own foodgroup in order to delete items.');}\
         </script><body onload='myFunction()'>"
    if request.method == 'POST':
